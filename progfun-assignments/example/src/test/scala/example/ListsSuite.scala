...
class ListsSuite extends FunSuite {
  ...
  test("sum of a few numbers with negative") {
    assert(sum(List(1,-4,0)) === -3)
  }

  test("sum of a few repeated elements") {
    assert(sum(List(1,2,0,2)) === 5)
  }
  ...
  test("max of a few numbers with negative") {
    assert(max(List(3, 7, -14, 2)) === 7)
  }

  test("max of a few numbers with repeated elements") {
    assert(max(List(3, 7, 2, 7)) === 7)
  }
