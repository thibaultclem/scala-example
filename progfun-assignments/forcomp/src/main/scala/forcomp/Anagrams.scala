...
object Anagrams {
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    (for (
      occx <- x
    ) yield (occx._1, if (yMap contains occx._1) occx._2 - yMap(occx._1) else occx._2)).filter(_._2 > 0)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def sentAnagHelper(occurences: Occurrences): List[Sentence] = {
      if (occurences.isEmpty) {
        ...
        for {
          ...
          sentence <- sentAnagHelper(subtract(occurences, combination))
        } yield word :: sentence
      }
    }

    sentAnagHelper(sentenceOccurrences(sentence))
